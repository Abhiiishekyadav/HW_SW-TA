...
void CNN(const double flattenedImage[], int imageWidth, int imageHeight, double output[]) {
    ...
    int num_kernels1 = 120;
    int kernelWidth1 = 3;
    int kernelHeight1 = 3;
    int imagedepth1 = 3;
    char a_f = 'R';
    double conv1Output[30*30*120];
    ...
    addNoise(conv1Output,0.038951, 30*30*120 );
    ...
    addNoise(conv2Output,0.073124, 28*28*88);
   // convulation 3
    double conv3Output[26*26*136];
    convolution(conv2Output, convolution3_weights, conv3Output,28,28,88, 3,3 , convolution3_bias,136,a_f);
    addNoise(conv3Output,0.091862, 26*26*136);
    // convulation 4
    double conv4Output[24*24*54];
    convolution(conv3Output, convolution4_weights, conv3Output,26,26,136, 3,3 , convolution4_bias,54,a_f);
    addNoise(conv4Output,0.012345, 24*24*54);
    // convolution 5
    double conv5Output[22*22*100];
    convolution(conv4Output, convolution5_weights, conv5Output,24,24,54, 3,3 , convolution5_bias,100,a_f);
    addNoise(conv5Output,0.057829,22*22*100);
    // convolution 6
    double conv6Output[20*20*96];
    convolution(conv5Output, convolution6_weights, conv6Output,22,22,100, 3,3 , convolution6_bias,96,a_f);
    addNoise(conv6Output,0.089712,20*20*96);
    // convolution 7
    double conv7Output[18*18*32];
    convolution(conv6Output, convolution7_weights, conv7Output,20,20,96, 3,3 , convolution7_bias,32,a_f);
    addNoise(conv7Output,0.045678,18*18*32);
    // convolution 8
    double conv8Output[16*16*128];
    convolution(conv7Output, convolution8_weights, conv8Output,18,18,32, 3,3 , convolution8_bias,128,a_f);
    addNoise(conv8Output,0.032156,16*16*128);
    // fully connected layer. 
    char a_f2='S';
    double fully1output[10];
    fullyConnectedLayer(conv8Output,fully1output,dense1_weights,dense1_bias,32768,10,a_f2);
    char a_f3='S';
    if(a_f3=='S'){
